...
<div class="max-w-3xl mx-auto">
  ...
  <form method="POST" action="{{ route('patients.update', $patient) }}" class="space-y-4">
    ...
    <div>
      ...
      <input type="tel" name="phone" placeholder="+256700000000" value="{{ old('phone', $patient->phone) }}" class="w-full rounded border px-3 py-2" />
    ...
    <div>
      ...
      <select name="gender" class="w-full rounded border px-3 py-2">
        <option value="" {{ old('gender', $patient->gender) == '' ? 'selected' : '' }}>Select</option>
    ...
    <div>
      ...
      <select name="marital_status" class="w-full rounded border px-3 py-2">
        <option value="" {{ old('marital_status', $patient->marital_status) == '' ? 'selected' : '' }}>Select</option>
    ...
    <div>
      ...
      <textarea name="address" class="w-full rounded border px-3 py-2">{{ old('address', $patient->address) }}</textarea>
    </div>
    <details class="p-3 border rounded" open>
      ...
      <div class="mt-3 space-y-3">
        <div>
          ...
          <input name="emergency_contact_name" value="{{ old('emergency_contact_name', $patient->emergency_contact_name) }}" class="w-full rounded border px-3 py-2" />
        </div>
        <div>
          <label class="block text-sm font-medium">Emergency contact phone</label>
          <input type="tel" name="emergency_contact_phone" placeholder="+256700000000" value="{{ old('emergency_contact_phone', $patient->emergency_contact_phone) }}" class="w-full rounded border px-3 py-2" />
    ...
    <details class="p-3 border rounded">
      ...
      <div class="mt-3 space-y-3">
        <div>
          ...
          <textarea name="allergies" class="w-full rounded border px-3 py-2">{{ old('allergies', $patient->allergies) }}</textarea>
        ...
        <div>
          ...
          <textarea name="conditions" class="w-full rounded border px-3 py-2">{{ old('conditions', $patient->conditions) }}</textarea>
